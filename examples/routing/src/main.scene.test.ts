...
const home: Model = { route: { _tag: 'Home' } }
const people = (searchText: Option.Option<string>): Model => ({
  route: { _tag: 'People', searchText },
})
const person = (personId: number): Model => ({
  route: { _tag: 'Person', personId },
})
const nested: Model = { route: { _tag: 'Nested' } }
const notFound = (path: string): Model => ({
  route: { _tag: 'NotFound', path },
})

describe('routing scene', () => {
